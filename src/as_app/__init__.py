...
The version chain is the library's mechanism (:mod:`as_platform.version`); this package
supplies its own distribution name, ``VERSION`` path and placeholder, because those are
this repository's identity (ADR-0009 decision 2).
"""
...
#: Repository ``VERSION`` file, two directories above this module. It is the version source
...
def _version_file_version() -> str:
    ...
    Returns:
        The stripped contents of ``_VERSION_FILE``, or ``_UNKNOWN_VERSION`` when the file is
        not present — which is the case for an installed wheel.
    """
...
def _read_version() -> str:
    ...
    """
    return read_version(_DISTRIBUTION_NAME, _version_file_version)
