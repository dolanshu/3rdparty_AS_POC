...
`To` and `CSeq` belong to the dialog and the second leg has its own. **`Call-ID` is the
exception, and the distinction is between the header table and the call-control event**: it is
absent from `PASSTHROUGH_HEADERS`, but it is carried into the second leg inside the
`CCEventTry` the controller rebuilds, so it does **not** change across the B2BUA boundary —
measured, and stated in section 10.2. *That behaviour is a **Phase 1 defect**, not the design
intent: the sentence above (`Call-ID` belongs to the dialog and the second leg has its own) is
what the code is meant to do, and the maintainer ruled on 2026-09-19 that the reuse of the
inbound `Call-ID` is to be fixed — on `main`, merged back into `phase2` — before P9 resumes.
This paragraph is to be restored to the design intent when P9's design stage is redone; see
`docs/phase2-plan.md` section 3, P9.* `User-Agent` is the identity of the AS, and
`Content-Type` follows the body.

The second leg's own Call-ID has to be set explicitly here: sippy copies a non-`None`
Call-ID from the `CCEventTry` verbatim (`sippy/UacStateIdle.py`) and only its `CCB2BUA`
rewrites it (`sippy/b2bua.py`), and this project runs its own controller over a bare
`sippy.UA`, not `CCB2BUA`. `apply_call_policy` therefore derives a fresh `SipCallId` from
the trunk one by appending sippy's own suffix `-b2b_1`
(`B2BUA_CALL_ID_SUFFIX` in `src/as_app/sip_adapter.py`): route `1` is the AS's single
outbound leg, and every failover hop reuses the same value because the translated event is
built once and stored in `_pending_event`. The inbound `SipCallId` is never mutated — it is
the trunk leg's dialog identity. `CallController.call_id` stays the **trunk** Call-ID and
remains the log/trace correlation key across both legs; it is the wire Call-ID of the
outbound leg that differs.
...
Neither AS imports the other, no code is shared to make the chain work, and no existing
module changes behaviour. The two hops are already decided by configuration
(ADR-0008 decision 1):
...
### 10.2 Inherited constraint: the dialog `Call-ID` is preserved across both B2BUAs

> **Pending rework — this subsection describes the current, defective behaviour (maintainer
> ruling, 2026-09-19).** The preservation measured below is real, but the maintainer has ruled
> it a **Phase 1 defect**, not accepted behaviour: the AS reuses the inbound `Call-ID` on its
> outbound leg, contradicting the design intent stated in section 2.3 (`Call-ID` belongs to
> the dialog and the second leg has its own). The fix lands on **`main`** and is merged back
> into `phase2`; **P9 is paused** until then, and this subsection — and the section 2.3
> correction above — are to be **reworked when P9's design stage is redone**
> (`docs/phase2-plan.md` section 3, P9). The measurement is kept: it is a true observation of
> the code as it stands.

`REQ-NF-016` and `docs/phase2-plan.md` section 6 assume the opposite — *"two B2BUAs in series
produce two different Call-IDs"* — and treat cross-AS correlation as unsolved. **Measured on
this machine, that premise is wrong for this codebase**: one `Call-ID` spans the trunk, AS-2's
trunk leg and the core leg (`distinct Call-IDs: 1`, ADR-0008 *Verified facts*). Section 2.3
carries the corrected header statement.

The mechanism is two stack behaviours and one decision both controllers already make:
...
UasStateIdle.recvEvent      self.ua.cId = self.ua.uasResp.getHFBody('call-id')
        ...
        │       CCEventTry((original[0], original[1], translated, ...))   # [0] kept
        │
        └── anti_fraud_as.FraudCallController._originate_allowed
                CCEventTry(event.getData())                                # [0] kept
        │
UacStateIdle.recvEvent      if cId == None: self.ua.cId = SipCallId()
                            else:           self.ua.cId = cId.getCopy()   # reuse, not regenerate
```

Consequences for the implementation, stated so they are not rediscovered:

- **A chained call is one correlated trace, not two.** Both instances key their trace, their
  structured log and their console feed by the same `Call-ID`, so cross-AS correlation needs
  no mechanism built for it.
- **The property is relied upon, not mandated.** Nothing declares it a contract, and it is
  not the pass-through set that produces it (`Call-ID` is absent from `PASSTHROUGH_HEADERS`).
  Any change to how a controller rebuilds `CCEventTry`, or a sippy upgrade that changes
  `UacStateIdle`, can break it silently; `tools/chained_as_probe.py` is the guard that turns
  that into a failure. Production correlation uses `P-Charging-Vector`'s ICID, which *is* in
  the pass-through set (ADR-0008, *Gaps accepted*).
- **The requirements text is corrected in the implementation commit**, not here — see section
  10.5.
...
- **The `Call-ID` per hop.** It prints the Call-ID the S-CSCF used, the Call-ID AS-2 saw and
  the Call-ID the core saw, so the preservation of section 10.2 is demonstrated rather than
  claimed.
...
| New gaps accepted | `docs/production-gaps.md` (no iFC/ISC emulation; Call-ID preservation relied upon but not mandated; no shared state; catalogue coupling; no chain failure/ordering semantics) |
...
**The two "Corrected `Call-ID` premise" rows that used to stand here are withdrawn.** They
assigned P9's implementation commit to reword `REQ-NF-016` / `REQ-F-028` and the plan's
section 6 ("Chained Call-IDs") and section 3 P9 ("Known issue") to the measured one-Call-ID
behaviour. The maintainer ruled on 2026-09-19 that the reuse of the inbound `Call-ID` is a
**Phase 1 defect**, not accepted behaviour, so those rows describe a change to *intended*
behaviour and **do not stand**; the requirements and plan wording are not changed, and this
design is pending rework (`docs/phase2-plan.md` section 3, P9).
