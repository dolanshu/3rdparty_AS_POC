...
| `as_platform/route_header.py` | Parses the top `Route` on the trunk INVITE for the outbound wire destination (both AS instances) |
...
`Record-Route`, `Contact`, `Max-Forwards` and `Content-Length` are hop by hop — the inbound
`Route` set is **not** copied to the outbound leg, but its **top entry** selects the wire
destination (S-SBC return) per RFC 3261 section 8.1.1.1; `From`,
...
| `FRAUD_SBC_PEER_ADDRESS` | `127.0.0.1` | Fallback next hop when the trunk INVITE has no `Route` |
| `FRAUD_SBC_PEER_PORT` | `15062` | UDP port of that fallback hop (mock S-SBC return for `s-sbc-mock-fraud`) |
...
### 10.6 P9b rework — iFC orchestrator mock (ADR-0014, 2026-09-23)

P9b **supersedes** the trunk-to-trunk wiring of section 10.1. AS instances still do not
import each other; the chain is wired in `src/ims_mock/` plus extensions to `s_sbc_mock`.

| Module | Responsibility |
| --- | --- |
| `ims_mock/chain_config.py` | Ordered iFC list: anti-fraud then translation |
| `ims_mock/orchestrator.py` | Pure state machine (unit-tested) |
| `ims_mock/chained_stack.py` | Per-session orchestrator, wires AS-1/AS-2, S-SBC, P-CSCF, terminating UAS |
| `ims_mock/pcscf_relay.py` | Forwards AS-2 outbound INVITE toward terminating UAS |
| `ims_mock/terminating_uas.py` | Called-party UAS (reuses `CoreUas` answer pattern) |
| `tools/chained_helpers.py` | Shared bootstrap for demo, probe and `chained_pair_factory` |

| Hop | Mechanism |
| --- | --- |
| Subscriber → AS-1 | `TrunkUac.place_call` (iFC #1) with `Route` to S-SBC return |
| AS-1 allow → iFC #2 | Return-side **透传** callback → `TrunkUac.send_trunk_invite` to AS-2 |
| AS-2 allow → terminating | Return **透传** → `PcscfRelay` → `TerminatingUas` |
| `*_SBC_PEER_*` | S-SBC **return** port only (fallback when no `Route`) |

**Four AS-leg Call-IDs** on the allow path: AS-1 trunk `X`, AS-1 outbound `X-b2b_1`, AS-2
trunk `Z` (iFC #2), AS-2 outbound `Z-b2b_1`. Cross-AS `Call-ID` correlation remains unsolved
(`REQ-NF-016`); ICID is passed through and preserved.
