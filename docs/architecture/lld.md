...
measured, and stated in section 10.2. `User-Agent` is the identity of the AS, and
...
| Corrected `Call-ID` premise | `docs/requirements/functional-and-nonfunctional.md` — `REQ-NF-016` and `REQ-F-028` currently assert that two B2BUAs produce two Call-IDs; reword to the measured behaviour (one `Call-ID` spans the chain) while keeping `REQ-F-028`'s intent, per-instance observability |
| Corrected `Call-ID` premise | `docs/phase2-plan.md` — section 6 ("Chained Call-IDs") and section 3 P9 ("Known issue") say the same thing and are corrected with the same wording |
