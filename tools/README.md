...
| `tools/chained_as_probe.py` | Probes the **iFC-orchestrated** chain (`ims_mock`, ADR-0014): four AS-leg `Call-ID`s, ICID preservation, allow and 608 short-circuit. Design instrument, not a test. |
...
| `tools/demo_chained_call.py` | Narrated guard for `make demo-chained`: iFC chain via `tools/chained_helpers.py` (ADR-0014). Asserts four AS-leg `Call-ID`s, ICID, allow path and 608 short-circuit; exits non-zero on mismatch. |
